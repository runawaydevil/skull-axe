...
function atualiza_descriptor($caminho) {
	...
	if (!is_file(blogparm('DRAFTSDIR').$caminho)) {
		axe_error('Não existe o arquivo '.blogparm('DRAFTSDIR').$caminho,E_USER_ERROR);
		die;
	} else {
		include blogparm('DRAFTSDIR').$caminho;
		loadpostvars();
		if (!(isset($POST['POSTTIME']) && isset($POST['POSTNAME']))) {
			axe_error('Impossível atualizar: Não há POSTTIME ou POSTNAME no arquivo '.blogparm('DRAFTSDIR').$caminho,E_USER_ERROR);
			die;
		} else if (!already_exists(substpostvars('%%POSTNAME%%'))) {
			axe_error('Impossível atualizar: não existe o nome '.substpostvars('%%POSTNAME%%')." no catálogo principal.",E_USER_ERROR);
			die;
		} else {
			$saida="<?php\n";	
			foreach ($POST as $key => $value) {
...
function cria_descriptor($caminho) {
	...
	if (!is_file(blogparm('DRAFTSDIR').$caminho)) {
		...
		loadpostvars();
		if ((isset($POST['POSTTIME']) && isset($POST['POSTNAME']))) {
			axe_error('Impossível criar: Já há POSTTIME e POSTNAME no arquivo '.blogparm('DRAFTSDIR').$caminho,E_USER_ERROR);
			die;
		} else if (!isset($POST['POSTTITLE'])) {
			axe_error('Impossível criar: não há POSTTITLE definido no arquivo '.blogparm('DRAFTSDIR').$caminho,E_USER_ERROR);
			die;
		} else {
			...
			if (already_exists(substpostvars('%%POSTNAME%%'))) {
				...
				axe_error("Impossível criar descriptor para $caminho: já existe o nome ".substpostvars('%%POSTNAME%%')." no catálogo principal. Draft renomeado para $randpref$caminho",E_USER_ERROR);
				die;
			} else {
				$saida="<?php\n";	
				foreach ($POST as $key => $value) {
